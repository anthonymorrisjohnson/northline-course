...
| Exhibit E | `northline/agents/triage/exhibit_e.json` | The fifteen night texts, verbatim, with their timestamps. They are also seeded into the corpus, so they show up in the classification as well as in the acceptance test. |
...
| The disengaged cohort | `patients_inactive_after_escalation` in `northline/pm/out/queue_metrics.json` | The board's 334 becomes 342 when counted from the queue log: patients who stopped responding after an escalation nobody answered. It is also a row on the dashboard and a trend chart. |
