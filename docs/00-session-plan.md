...
Slide numbers are for the browser deck at `/slides` and its PowerPoint export, `slides/present.pptx`. After editing `slides/present.html`, rebuild the export with `uv run --with python-pptx python scripts/export_deck.py`. `slides/present-editable.pptx` is the same sixteen slides as native text, shapes and tables, in Arial, for editing in PowerPoint, Keynote or Google Slides; rebuild it with `uv run --with python-pptx --with beautifulsoup4 --with pillow python scripts/export_deck_editable.py`. `slides/deck.pptx` is the earlier editable PowerPoint deck and has its own numbering.

**The follow-along deck.** `/follow` (and `slides/follow-along.pptx`) is a second, twenty-two-slide deck you can present from the main screen, and the one to give anyone without a laptop: four slides of basics on agents, tools and MCP, a five-minute team discussion on which door fits the patient and which fits Prairie, then each step of the do-along with what to type, ending with the six-question framework to take home, a real capture of what comes back, and the narrative that goes with it. Send it with the pre-work, and put it on a second screen if the room has one. Its text lives in `slides/follow-along.body.html`; after editing, run `uv run python scripts/build_follow_along.py`, then `uv run --with python-pptx python scripts/export_deck.py follow-along`.
