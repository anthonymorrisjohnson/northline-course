...
7. Point at every MISS, MISROUTED, and PROMISED. If there are none, say so in one line: the defaults pass the key, and the interesting table only appears when a threshold is loosened or consent is honoured. Read out the after-hours line at the bottom of the table (the night-time wait their rule implies); that number is the only place decision 3 shows up before the dashboard. For each trap message that missed (5, 9, 12, 13), quote the message and the nurse's note from `northline/agents/triage/nurse_key.json`. Say plainly that the test informs the decision and does not block it: a miss here becomes missed urgent cases on the dashboard, and the deployer owns that. Then ask: "Do you deploy it as is, or change a decision and re-test?" If they change a decision, go back to step 5 for that decision only. Do not loop more than twice; a third time, deploy as is and say why.
8. Run `uv run python -m northline.agents.triage.deploy`. Then say: open http://127.0.0.1:8765/dashboard. If the server is not running, tell them to start it in a separate terminal window with `uv run python -m northline.agent.server` and leave that window open; do not start it from here, it would block or be killed. Tell them what to look at: the Next quarter column, the "Urgent response" row (their after-hours rule), the nurse response line after week 40, the "urgent cases missed" tile, and the Sign Prairie toggle.
